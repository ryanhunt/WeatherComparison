...
class AccuWeather():
	...
	def getLocationIDFromCoords(self, lat, long):
		...
		#
		url = 'http://api.accuweather.com/locations/v1/cities/geoposition/search?apikey=%s&q=%s%s%s' % (self.apiKey, lat, "%2C%20",long)
		#print(url)
	...
	def getCurrentConditions(self, key):
		...
		#
		url = 'http://api.accuweather.com/currentconditions/v1/%s?apikey=%s' % (key, self.apiKey)
		#print(url)
		try:
			with urllib.request.urlopen(url) as url:
				data = json.loads(url.read().decode())
